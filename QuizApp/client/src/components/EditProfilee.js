...
import { fetchUser, updateUserr, setUserField, setNestedUserField, setArrayUserField } from '../pages/slices/userSlice';
import Sidebarcan from "../pages/canspace/Sidebar/Sidebarcan";
...
function EditProfilee() {
  ...
  const deletePhoto = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/auth/${canId}/photo`);
      dispatch(fetchUser(canId));
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(updateUserr({ canId, userData: localUser })).unwrap();
    } catch (err) {
